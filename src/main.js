(() => {
  ...
  const loaderPhaseMarks = $$('#loaderPhases span');
  ...
  const envelopeRecipient = $('#envelopeRecipient');
  const envelopeRecipientNote = $('#envelopeRecipientNote');
  const mailCardEyebrow = $('#mailCardEyebrow');
  const mailCardGuest = $('#mailCardGuest');
  const mailCardGuestName = $('#mailCardGuestName');
  const mailCardLead = $('#mailCardLead');
  const finaleEyebrow = $('#finaleEyebrow');
  const finaleLead = $('#finaleLead');
  const inviteCardKicker = $('#inviteCardKicker');
  const inviteCardGuest = $('#inviteCardGuest');
  const inviteCardGuestName = $('#inviteCardGuestName');
  const inviteCardTitle = $('#inviteCardTitle');
  const inviteCardLead = $('#inviteCardLead');
  const inviteEdition = $('#inviteEdition');
  ...
  const guestName = normalizeGuestName(
    invitationParams.get('guest') || invitationParams.get('to') || invitationParams.get('name') || ''
  );

  function initGuestCopy() {
    const baseTitle = 'آیین دانش‌آموختگی پزشکی بهمن 1398';
    if (!guestName) return;

    body.classList.add('has-guest');
    if (guestName.length > 34) body.classList.add('guest-name-long');

    document.title = `${guestName} · ${baseTitle}`;
    if (loaderTitle) loaderTitle.textContent = `${guestName}، این نامه برای شماست`; 
    const description = `${guestName}؛ با احترام، از شما برای حضور در آیین دانش‌آموختگی دانشجویان پزشکی ورودی بهمن ۱۳۹۸ دانشگاه علوم پزشکی مازندران دعوت می‌کنیم.`;
    document.querySelector('meta[name="description"]')?.setAttribute('content', description);
    document.querySelector('meta[property="og:title"]')?.setAttribute('content', `${guestName} · آیین دانش‌آموختگی پزشکی`);
    document.querySelector('meta[property="og:description"]')?.setAttribute('content', description);

    if (envelopeRecipient) envelopeRecipient.textContent = guestName;
    if (envelopeRecipientNote) envelopeRecipientNote.textContent = 'از سوی دانشجویانی که افتخار شاگردی شما را داشته‌اند';

    if (mailCardEyebrow) mailCardEyebrow.textContent = 'به احترام دانشی که از شما آموختیم';
    if (mailCardGuest) mailCardGuest.hidden = false;
    if (mailCardGuestName) mailCardGuestName.textContent = guestName;
    if (mailCardLead) mailCardLead.textContent = 'سال‌های دانشجویی ما تنها با کتاب و آزمون ساخته نشد؛ بخشی از آن در کلاس‌ها، بخش‌ها و لحظه‌هایی شکل گرفت که از نگاه، صبوری و اعتماد شما آموختیم. امروز که به پایان این مسیر رسیده‌ایم، دوست داریم شادیِ آغاز فصل تازه را در حضور شما تجربه کنیم؛ استادی که سهمی ماندگار در این راه داشته است.';
    mailCard?.setAttribute('aria-label', `دعوت آیین دانش‌آموختگی برای ${guestName}`);

    if (finaleEyebrow) finaleEyebrow.textContent = 'با سپاس از سال‌هایی که از شما آموختیم';
    if (finaleLead) finaleLead.textContent = 'برای ما این جشن فقط پایان سال‌های تحصیل نیست. پشت این روز، خاطره‌ی کلاس‌ها، بخش‌ها، پرسش‌ها، اشتباه‌ها و لحظه‌هایی است که با راهنمایی شما به آموختن تبدیل شدند. بخشی از پزشکی که امروز می‌شناسیم، از شما به ما رسیده است؛ و چه زیباست اگر آغاز راه حرفه‌ای‌مان را در حضور شما جشن بگیریم.';

    if (inviteCardKicker) inviteCardKicker.textContent = 'با احترام و سپاس';
    if (inviteCardGuest) inviteCardGuest.hidden = false;
    if (inviteCardGuestName) inviteCardGuestName.textContent = guestName;
    if (inviteCardTitle) inviteCardTitle.textContent = 'خوشحال می‌شویم این روز را در کنار شما به یاد بسپاریم';
    if (inviteCardLead) inviteCardLead.textContent = 'با احترام و سپاس، از شما دعوت می‌کنیم در آیین دانش‌آموختگی ما در کنارمان باشید. روزی که امروز به آن رسیده‌ایم، حاصل سال‌هایی است که در کنار استادان‌مان آموختیم چگونه دانش را با مسئولیت، دقت و انسانیت همراه کنیم. حضور شما برای ما معنایی فراتر از یک مهمان دارد؛ دیدن شما، این پایان شیرین و آغاز تازه را به خاطره‌ای کامل‌تر و ماندگارتر تبدیل خواهد کرد.';
    if (inviteEdition) inviteEdition.textContent = '۵ مهر ۱۴۰۵ · ساری';
    inviteStage?.setAttribute('aria-label', `دعوت آیین دانش‌آموختگی برای ${guestName}`);
    applyAdaptiveGuestLayout();
  }


  function fitTextToLines(el, { min = 12, max = 40, maxLines = 2, step = 1, cacheKey = '' } = {}) {
    if (!el || !el.isConnected) return max;
    const key = `${cacheKey}:${viewportW}:${guestName}`;
    if (el.dataset.fitKey === key && el.dataset.fitSize) return Number(el.dataset.fitSize) || max;
    el.style.setProperty('font-size', `${max}px`, 'important');
    const measureOverflow = () => {
      const cs = getComputedStyle(el);
      const lineHeight = parseFloat(cs.lineHeight) || Math.max(1, Number(cs.fontSize.replace('px', '')) * 1.3);
      const lines = Math.max(1, Math.round(el.getBoundingClientRect().height / lineHeight));
      return lines > maxLines || el.scrollWidth > el.clientWidth + 2 || el.scrollHeight > el.clientHeight + 2;
    };
    let size = max;
    for (let guard = 0; guard < 64 && size > min && measureOverflow(); guard += 1) {
      size -= step;
  ...
  function applyAdaptiveGuestLayout() {
    if (!guestName) return;
    requestAnimationFrame(() => requestAnimationFrame(() => {
      [loaderTitle, envelopeRecipient, envelopeRecipientNote, mailCardGuestName, inviteCardGuestName].filter(Boolean).forEach((el) => {
        delete el.dataset.fitKey;
        delete el.dataset.fitSize;
      });
      fitTextToLines(loaderTitle, {
        min: viewportW <= 480 ? 26 : 30,
        max: viewportW <= 480 ? 38 : (viewportW <= 820 ? 44 : 52),
        maxLines: viewportW <= 640 ? 3 : 2,
        step: .45,
        cacheKey: 'loaderTitle'
      });
      const envSize = fitTextToLines(envelopeRecipient, {
        min: viewportW <= 480 ? 14 : 18,
        max: viewportW <= 480 ? 22 : (viewportW <= 820 ? 26 : 32),
        maxLines: 3,
        step: .35,
        cacheKey: 'envelopeRecipient'
      });
      fitTextToLines(envelopeRecipientNote, {
        min: viewportW <= 480 ? 8.1 : 8.8,
        max: viewportW <= 480 ? 10.2 : 11.1,
        maxLines: 2,
        step: .2,
        cacheKey: 'envelopeRecipientNote'
      });
      fitTextToLines(mailCardGuestName, {
        min: viewportW <= 480 ? 6.2 : 7.2,
        max: viewportW <= 480 ? 10.2 : (viewportW <= 820 ? 11.8 : 14.2),
        maxLines: 2,
        step: .35,
        cacheKey: 'mailCardGuestName'
      });
      const inviteSize = fitTextToLines(inviteCardGuestName, {
        min: viewportW <= 480 ? 17 : 19,
        max: viewportW <= 480 ? 28 : (viewportW <= 820 ? 30 : 34),
        maxLines: viewportW <= 480 ? 3 : 2,
        step: .35,
        cacheKey: 'inviteCardGuestName'
      });
      const dense = envSize <= (viewportW <= 480 ? 17.5 : 21) || inviteSize <= (viewportW <= 480 ? 20 : 23) || guestName.replace(/\s+/g, '').length > 24;
      body.classList.toggle('guest-name-dense', dense);
    }));
  }
  ...
  const audioManifest = [
    backgroundMusic?.querySelector('source')?.getAttribute('src') || backgroundMusic?.getAttribute('src') || 'assets/mazums-elegant-ambient.mp3',
    heartbeatAudio?.querySelector('source')?.getAttribute('src') || heartbeatAudio?.getAttribute('src') || 'assets/mazums-heartbeat-72bpm.mp3'
  ];
  ...
  function setLoader(p, status = '') {
    ...
    requestAnimationFrame(() => {
      ...
      if (v !== loaderShownValue) {
        ...
        if (loaderPhaseMarks.length) {
          const a = next >= .12;
          const b = next >= .44;
          const c = next >= .82;
          loaderPhaseMarks[0]?.classList.toggle('is-done', a);
          loaderPhaseMarks[1]?.classList.toggle('is-done', b);
          loaderPhaseMarks[2]?.classList.toggle('is-done', c);
        }
      }
  ...
  function closeLoader() {
    ...
    setLoader(1, 'همه‌چیز برای آغاز این روایت آماده است');
  ...
  function preloadMediaElement(el, { timeout = 6000 } = {}) {
    if (!el) return Promise.resolve(false);
    if (el.dataset.preloadState === 'done') return Promise.resolve(true);
    if (el._preloadPromise) return el._preloadPromise;
    el._preloadPromise = withTimeout(new Promise((resolve) => {
      let settled = false;
      const finish = (ok) => {
        if (settled) return;
        settled = true;
        el.dataset.preloadState = ok ? 'done' : 'failed';
        cleanup();
        resolve(!!ok);
      };
      const cleanup = () => {
      ...
      try { el.load?.(); } catch {}
      if ((el.readyState || 0) >= 2) finish(true);
    }), timeout).then(Boolean).finally(() => {
      delete el._preloadPromise;
    });
    return el._preloadPromise;
  }

  function warmCoreAudio() {
    const run = async () => {
      await Promise.allSettled([
        preloadMediaElement(backgroundMusic, { timeout: 7000 }),
        preloadMediaElement(heartbeatAudio, { timeout: 7000 })
      ]);
      warmServiceWorkerCache(audioManifest);
    };
    if (globalThis.scheduler?.postTask) {
      globalThis.scheduler.postTask(run, { priority: 'background' }).catch(() => run());
    } else if ('requestIdleCallback' in window) {
      requestIdleCallback(() => void run(), { timeout: 1400 });
    } else {
      setTimeout(() => void run(), 160);
    }
  }

  const mediaBlobJobs = new Map();
  const mediaObjectUrls = [];

  async function fetchBlobFully(url, onProgress) {
    if (mediaBlobJobs.has(url)) return mediaBlobJobs.get(url);
    const job = (async () => {
      let lastError;
      for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
          const response = await fetch(url, { cache: attempt === 0 ? 'force-cache' : 'reload', credentials: 'same-origin' });
          ...
          lastError = error;
          if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 550 * (attempt + 1)));
        }
      }
      throw lastError || new Error(`Unable to fetch ${url}`);
    })().catch((error) => {
      mediaBlobJobs.delete(url);
      throw error;
    });
    mediaBlobJobs.set(url, job);
    return job;
  }

  function waitForLocalMedia(el, minimumReadyState = 2) {
    return new Promise((resolve, reject) => {
      if (!el) return reject(new Error('Missing media element'));
      const cleanup = () => {
      ...
      const ready = () => {
        if ((el.readyState || 0) >= minimumReadyState) {
          cleanup();
          resolve(true);
        }
      };
      const fail = () => {
        cleanup();
        reject(el.error || new Error('Media decode failed'));
      };
      el.addEventListener('loadeddata', ready);
      el.addEventListener('loadedmetadata', ready);
      el.addEventListener('error', fail, { once: true });
      if ((el.readyState || 0) >= minimumReadyState) ready();
    });
  }

  async function attachCompleteBlob(el, blob) {
    if (!el || !blob) throw new Error('Cannot attach media blob');
    const objectUrl = URL.createObjectURL(blob);
    mediaObjectUrls.push(objectUrl);
    el.src = objectUrl;
    el.preload = 'auto';
    try { el.load(); } catch {}
    await waitForLocalMedia(el, 2);
    return true;
  }

  async function prepareCompleteAV(onProgress) {
    const sources = [
      { key: 'video', url: heroVideoSource(), el: heroVideo, weight: .72 },
      { key: 'music', url: audioManifest[0], el: backgroundMusic, weight: .20 },
      { key: 'heartbeat', url: audioManifest[1], el: heartbeatAudio, weight: .08 }
    ];
    const progress = new Map(sources.map((s) => [s.key, 0]));
    const report = () => onProgress?.(sources.reduce((sum, s) => sum + (progress.get(s.key) || 0) * s.weight, 0));
    const blobs = await Promise.all(sources.map(async (source) => {
      const blob = await fetchBlobFully(source.url, (p) => {
        if (p > 0) progress.set(source.key, Math.max(progress.get(source.key) || 0, p));
        report();
      });
      progress.set(source.key, 1);
      report();
      return { ...source, blob };
    }));
    for (const item of blobs) await attachCompleteBlob(item.el, item.blob);
    heroReady = true;
    heroVideoFailed = false;
    heroVideoLoadPromise = Promise.resolve(true);
    sectionMediaReady.add(0);
    onProgress?.(1);
    return true;
  }

  async function preloadAllVisualAssets(onProgress) {
    const domVisuals = [
      ...$$('img[src]').map((el) => el.getAttribute('src')),
      ...$$('video[poster]').map((el) => el.getAttribute('poster'))
    ].filter(Boolean);
    const urls = [...new Set([...criticalManifest, ...warmManifest, ...sectionMediaManifest.flat(), ...domVisuals].filter(Boolean))];
    let done = 0;
    let cursor = 0;
    const failures = [];
    const concurrency = Math.max(2, Math.min(networkConcurrency(), 4));
    const workers = Array.from({ length: Math.min(concurrency, Math.max(1, urls.length)) }, async () => {
      while (cursor < urls.length) {
        const url = urls[cursor++];
        let ok = await preloadImage(url, { priority: 'high', decode: true }).catch(() => false);
        if (!ok) {
          mediaJobs.delete(url);
          await new Promise((resolve) => setTimeout(resolve, 320));
          ok = await preloadImage(url, { priority: 'high', decode: true }).catch(() => false);
        }
    ...
    await Promise.all(workers);
    if (failures.length) throw new Error(`Visual assets failed: ${failures.join(', ')}`);
    sectionMediaManifest.forEach((_, index) => sectionMediaReady.add(index));
    onProgress?.(1);
    return true;
  }

  window.addEventListener('beforeunload', () => {
    mediaObjectUrls.forEach((url) => { try { URL.revokeObjectURL(url); } catch {} });
  });
  ...
  async function warmSectionMedia(index, { guarded = false } = {}) {
    ...
    const job = (async () => {
      ...
      if (allReady) sectionMediaReady.add(index);
      else sectionMediaJobs.delete(index);
      return allReady;
    })().finally(() => {
  ...
  function warmVisualAssets() {
    const run = async () => {
      ...
      warmServiceWorkerCache([...criticalManifest, ...warmManifest, ...audioManifest, heroVideoSource()]);
    };
    if (globalThis.scheduler?.postTask) {
      globalThis.scheduler.postTask(run, { priority: 'background' }).catch(() => run());
    } else if ('requestIdleCallback' in window) {
  ...
  function initMailEntrance() {
    ...
    async function openMail(event) {
      ...
      try {
        ...
        // Main reading pause: keep the actual invitation face visible while the
        // card is fully extracted from the envelope. This is intentionally the
        // longest still moment in the entrance sequence.
        mailCard.classList.add('is-reading-hold');
        const invitationReadingHoldMs = reduceMotion ? 5200 : (innerWidth <= 820 ? 11500 : 9800);
        await wait(invitationReadingHoldMs);
        mailCard.classList.remove('is-reading-hold');
        await wait(180);
        ...
        // The cinematic portal is only a transition beat; do not create a
        // second reading pause here. The long pause belongs to the invitation.
        await wait(180);
        body.classList.add('invitation-entered');
  ...
  function showBootError(error) {
    setLoader(Math.min(.94, Math.max(.08, loaderShownValue / 100 || .08)), 'اتصال کامل نشده؛ برای حفظ کیفیت روایت وارد صفحه نمی‌شویم.');
    if (loaderErrorText) loaderErrorText.textContent = 'دریافت یکی از فایل‌های ضروری کامل نشد. اتصال اینترنت را بررسی کنید و دوباره تلاش کنید.';
    if (loaderError) loaderError.hidden = false;
    if (loaderRetry) loaderRetry.disabled = false;
  }

  async function init() {
    if (bootInProgress || loaderClosed) return;
    bootInProgress = true;
    if (loaderError) loaderError.hidden = true;
    if (loaderRetry) loaderRetry.disabled = true;
    initGuestCopy();
    const startedAt = performance.now();
    const boot = { visuals: 0, fonts: 0, av: 0 };
    const paintBoot = (status) => {
      const progress = .02 + boot.fonts * .10 + boot.visuals * .30 + boot.av * .56;
      setLoader(Math.min(.985, progress), status);
    };

    try {
      setLoader(.02, 'در حال آماده‌سازی کامل نامه و رسانه‌ها…');
      const fontsTask = waitForFonts((p) => {
        boot.fonts = p;
        paintBoot(p < 1 ? 'در حال آماده‌سازی تایپوگرافی فارسی…' : 'تایپوگرافی آماده است');
      });
      const visualsTask = preloadAllVisualAssets((p) => {
        boot.visuals = p;
        paintBoot(p < 1 ? 'در حال دریافت و آماده‌سازی تصاویر روایت…' : 'تمام تصاویر آماده‌اند');
      });
      const avTask = prepareCompleteAV((p) => {
        boot.av = p;
        paintBoot(p < 1 ? 'در حال دریافت کامل ویدیو و صداها…' : 'ویدیو و صداها کامل روی دستگاه آماده‌اند');
      });

      await Promise.all([fontsTask, visualsTask, avTask]);
      applyAdaptiveGuestLayout();
      const remaining = Math.max(0, 700 - (performance.now() - startedAt));
      if (remaining) await new Promise((resolve) => setTimeout(resolve, remaining));
      setLoader(.995, 'همه‌ی رسانه‌ها کامل آماده‌اند؛ نامه اکنون قابل گشودن است');
      ...
      warmServiceWorkerCache([...criticalManifest, ...warmManifest, ...audioManifest, heroVideoSource(), ...sectionMediaManifest.flat()]);
    } catch (error) {
      showBootError(error);
